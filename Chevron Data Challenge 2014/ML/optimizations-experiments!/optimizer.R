...
setwd("~/Chevron/DSChallenge/optimizations")
...
# the model - lm
fit <- lm(EUR_o..Mstb. ~. , data = train, na.action = na.pass)
...
# optimization first well in the data!
stub <- data[1, ]
stub

nonactionables <- c("Operator", "County", "Completion.Year", "Deepest_Zone", "Depth.Top")

# create optimization wrapper for EUR
opt_gm <- function(x) {
  z <- stub
  
  # copy the data over top of the stub record
  for (i in names(stub[2:11])) {
    if (!(i %in% nonactionables)) {
      z[[i]] <- x[[i]]
    }
  }
  # score the data and return the negative
  y <- (-1 * predict(fit,z))
  print(paste(str(z),y,sep = " ===================> "))
  y
}
