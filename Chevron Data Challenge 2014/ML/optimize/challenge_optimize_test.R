...
setwd("~/Chevron/DSChallenge/preprocess")
...
for(i in names(test)){
  ...
  if (noOfCats == 1) {
    test[[i]] <- NULL
  } else if ((noOfCats <= length(test[[i]])/2 && types[[i]] == "factor") || types[[i]] == "character") {
    print(paste("processing ====", i, sep = "> "))
    means <- sapply(split(test$EUR_o..Mstb., test[[i]]), mean)
    print(names(means))
    # convert to character type
    test[[i]] <- as.character(test[[i]])
    for(j in names(means)) {
      print(paste("replacing", j, "by", means[[j]], sep = " "))
      test[[i]][test[[i]] == j] <- means[[j]]
    }
...
# training set
# training <- read.csv("data_imputed5.csv", header = TRUE)
names <- names(data)
names
...
# replace the NAs by mean
# for(i in names(data)){
#   data[[i]][is.na(data[[i]])] <- mean(data[[i]], na.rm = TRUE)
# }
# write.table(data, "data_test_imputed5.csv", sep=",", row.names=FALSE, quote = FALSE)

# the model - M5P from RWeka package
# fit <- M5P(EUR_o..Mstb. ~. , data = training, na.action = na.pass)
forecasts <- predict(best.fit, newdata = testdata)
forecasts <- as.data.frame(forecasts)
colnames(forecasts) <- "EUR_o..Mstb."
#Replacing NA with average EUR from training set (NA not allowed for submission)
# predicted[is.na(predicted)] <- mean(training$EUR_o..Mstb.)
