...
setwd("~/Chevron/DSChallenge/nested-learning/binary")
...
CUTOFF <- 0.96
marks <- as.numeric(quantile(data$EUR_o..Mstb., c(0, CUTOFF, 1)))
marks
...
copy <- data
copy$Class <- cut(x = copy$EUR_o..Mstb., breaks = marks, labels = FALSE, include.lowest = T) # 
copy$Class <- factor(copy$Class)
# str(copy)
table(copy$Class)

minors <- subset(copy, !Class %in% c(1))
table(minors$Class)
minors$Class <- cut(x = minors$EUR_o..Mstb., breaks = 2, labels = FALSE, include.lowest = T) # 
minors$Class <- factor(minors$Class)
table(minors$Class)

# increase cases of minors by upsampling!
set.seed(382)
# t <- SMOTE(Class ~ ., data = minors, k = 5) #, perc.over = 300, perc.under = 300, k = 5, learner = "rpartXse", se = 0.5
t <- upSample(minors[,-which(names(minors) %in% c("Class"))],minors$Class, list = F, yname = "Class")
table(t$Class)
t$Class <- revalue(t$Class, c("1"="2"))
# minors <- subset(t, !Class %in% c(1))
# table(minors$Class)
copy<-rbind(copy,t)

write.table(copy, "amplified.csv",sep=",",row.names=FALSE, quote = FALSE)

h <- hist(x = copy$EUR_o..Mstb., plot = T, breaks = marks)
h$counts

# the normal fit that we do!
theModel <- challengeModel(copy[,-which(names(copy) %in% c("Class"))])
summary(theModel)
...
# fSmote(100)
xmin <- optimize(fSmote, c(1, 1000), maximum = T) # , tol = 0.0001
xmin

# set.seed(xmin$maximum)
set.seed(382)
t <- SMOTE(Class ~ ., data = copy, k = 5) #, perc.over = 300, perc.under = 300, k = 5, learner = "rpartXse", se = 0.5
# s <- subset(t, !Class %in% c(1))
table(t$Class)
...
fDown(1000)
xmin <- optimize(fDown, c(1, 1000), maximum = T) # , tol = 0.0001
xmin
...
t <- copy[,-which(names(copy) %in% c("Class"))]
highCUMs <- t[t$EUR_o..Mstb.>marks[[2]] & t$EUR_o..Mstb.<marks[[3]], ]
str(highCUMs)
