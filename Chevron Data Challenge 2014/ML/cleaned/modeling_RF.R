...
best.test <- baseTrain
best.fit <- randomForest(formula = EUR_o..Mstb. ~. , data = baseTrain, na.action = na.exclude,  ntree = 100)
best.cor <- -1
...
for(i in 1:iterations) {
  ...
  # the model - M5P from RWeka package
  fit <- randomForest(EUR_o..Mstb. ~. , data = train, ntree = 100, na.action = na.exclude)  
