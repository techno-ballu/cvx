...
testing90.predicted <- predict(data.model.90, newdata = temp[which(!names(temp) %in% c(ID,METRIC))])
...
sorted <- data.imputed.cleaned.90[order(data.imputed.cleaned.90$CumProd),]
line <- gvisLineChart(xvar = ID, yvar = c("CumProd"), data = sorted)
plot(line)
hist(x = sorted$CumProd, plot = T)
# TODO change the cut-offs
LOW <- 0.40
HIGH <- 0.80
marks <- as.numeric(quantile(data.imputed.cleaned.90$CumProd, c(0, LOW, HIGH, 1)))
marks

sorted$Class <- cut(x = sorted$CumProd, breaks = marks, include.lowest = T, labels = F) 
table(sorted$Class)

# result <- buildRegression(data.imputed.cleaned.90)
data.cleaned <- data.imputed.cleaned.90
data.model <- data.imputed.model.90
str(data.cleaned)

# temp <- data.imputed.90[which(names(data.imputed.90) %in% names(data.cleaned.90))]
# temp <- data.imputed.cleaned.90
# # create a predictors sample having maximum dissimilarity from temp wells!
# set.seed(1134)
# # A random sample of 1 data points
# startSet <- sample(1:dim(temp)[1], 1)
# samplePool <- temp[-startSet, ]
# start <- temp[startSet, ]
# trainIndex <- maxDissim(start, samplePool, n = 100)
# wells.to.optimize=temp[trainIndex,]
...
optimums <- buildOptimizationsWithYConstraints(wells.to.optimize,data.model$finalModel)
